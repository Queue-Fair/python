---
## Queue-Fair Python Adapter README & Installation Guide
...
To add the Queue-Fair Client-Side JavaScript Adapter to your web server, you don't need the Python files included in this extension.
...
The Server-Side Adapter is a small Python library that will run when visitors access your site.  It periodically checks to see if you have changed your Queue-Fair settings in the Portal, but other than that if the visitor is requesting a page that does not match any queue's Activation Rules, it does nothing.
...
**2.** By default, Queue-Fair will cache your account settings in this folder.  If you want the cache to reside elsewhere, do
...
All client-modifiable settings are in the `QueueFairConfig` class.  You should never find you need to modify `queue_fair_adapter.py` - but if something comes up, please contact support@queue-fair.com right away so we can discuss your requirements.
